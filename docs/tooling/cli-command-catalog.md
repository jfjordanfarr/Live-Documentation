...
| `npm run live-docs:inspect` | Pathfinder CLI for querying dependency chains between artifacts. Supports symbol-level pathfinding using `file.ts#SymbolName` syntax. Use `--direction both` to search forward (outbound) and reverse (inbound) simultaneously. | `--from <path[#symbol]>`, `--to <path[#symbol]>`, `--direction <outbound|inbound|both>`, `--max-depth <n>`, `--json`, `--verbose` |
