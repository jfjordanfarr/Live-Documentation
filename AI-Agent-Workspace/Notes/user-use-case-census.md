...
[Progress: Dev Day 85 (2026-03-30.1) fixed Membrane Map refresh/share round-tripping by teaching the initial Explorer routing layer to understand compressed `?s=` state, so deep links now reopen the intended Membrane Map view and directory instead of falling back to Sources or stale localStorage.]
...
[This use case is the direct ancestor of the **Membrane Map** concept (Dev Day 79, 2026-03-22). Membrane Map formalizes the unified view continuum as a zoomable treemap with four rendering modes (Browse, Explore, Compare, Path). See [membrane-map.mdmd.md](../../.mdmd/layer-3/membrane-map.mdmd.md). Dev Day 80 (2026-03-23) began implementation: 12-step execution plan, 34 pure-math tests (layout, hierarchy, detail-levels, edge-bundling), DOM rendering, pin state management, focal overlay, SVG connections, URL state persistence via lz-string, and iterative visual playtesting with Playwright MCP. The four discrete modes were collapsed into a **continuous pin spectrum** (see UC-095). Dev Day 81 (2026-03-24) added pin-active dependency-flow layout (L→R columns via BFS + Kahn's topological sort), Internals pseudo-symbol absorption, LCA ancestor membranes with clickable escape-hatch navigation, and commitPrep.prompt.md codification. Dev Day 82 (2026-03-26) added FLIP position+scale animations for smooth visual transitions, deferred SVG connector drawing (await FLIP Promise), selection-only fast path via structural fingerprint, hover dimming (connection highlighting on symbol hover), and cross-column directory bands (trie-based hierarchical computation, Strategy B+C hybrid) where directory membranes span across dependency-flow columns with honest encapsulation and bare-band rendering for root-level files. Dev Day 83 (2026-03-27) tackled five visual/interaction refinements: hybrid mixed-content layout (Solution C — treemap for directories + card-grid for files, solving the area-crushing problem where files were ~1% of area), collapsed-by-default file cards with click-to-expand and pin-all, test-backed golden styling, `redrawConnections()` API for lightweight SVG refresh on tuning slider changes (CSS variable scoping fix + fixed-width 320px grid columns), and `markConnectedEndpoints()` for case-insensitive endpoint indication (green connected, blue pinned). Dead code removed (Click Behavior/Visual tuning sections). Dev Day 84 (2026-03-28) applied nine visual parity refinements: complete dimming model rewrite (layered opacity — baseline 0.3, undim pinned/connected to 1.0, hover further dims to 0.2), universal pin-all toggle on all cards in both renderers via `areAllSymbolsPinned()` helper, `__internals__` full participation (data attributes + pinned class + markConnectedEndpoints inclusion), reference badges from `publicSymbolsExtended`, auto-focus fix via `docRelativePath` (solving the "stray thin leaf" bug where tiny treemap tiles rendered as 9px floating overlays), `trySelectionOnlyUpdate` fast-path bypass for auto-focus conditions, "Open in Membrane Map" button in detail panel, detail panel suppression on initial load, and `clearHoverDimming` order-of-operations fix (persistent `--connected`/`--pinned` classes no longer stripped on mouseleave). Chat Archaeology confirmed no canvas exists (Dev Day 78 finding). Dev Day 85 (2026-03-30) advanced the Membrane Map from exploratory polish into durable surface behavior: browse mode gained an ancestor breadcrumb and collapsed-directory `Explore ⇗` affordance, hierarchy construction was corrected to use source-relative `node.id` rather than `.mdmd/layer-4/...` mirror paths, compressed `?s=` URLs began restoring Membrane Map state on refresh, a 16-test Playwright visual regression suite was authored from Dev Days 80–85 findings, and the pin-active band overflow bug was fixed by switching inner band grids from fixed `320px` columns to `1fr`. Primary remaining challenges are back-trace animation, bundle-edge progressive disclosure, detail-level wiring, `expandedCards` URL state, Membrane Map as default view, and the Layer 3 architecture doc update.]
...
### UC-096 — Browser-Level Visual Regression Backstop for Membrane Map (2026-03-30.1.md:L2698, L3043, L3222)
